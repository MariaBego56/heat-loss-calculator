    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100 p-4 pb-32">
      <div className="max-w-6xl mx-auto">
        ...
        <div className="bg-white rounded-lg shadow-lg p-6 mb-6">
          ...
          <div className="space-y-4">
            ...
            {/* Heat Source Details Section - ADD THIS */}
            <div className="border-t-2 pt-4 mt-4">
              <h3 className="font-semibold text-gray-800 mb-3">Heat Source Details</h3>
  
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-2">Heat Source Type</label>
                  <select value={heatSourceType} onChange={(e) => setHeatSourceType(e.target.value)}
                  className="w-full px-4 py-3 border-2 border-gray-300 rounded-lg focus:border-indigo-500 focus:outline-none">
                    {Object.keys(HEAT_SOURCE_TYPES).map(t => <option key={t} value={t}>{t}</option>)}
                  </select>
                </div>
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-2">Make</label>
                  <input type="text" placeholder="e.g. Worcester, Vaillant" value={heatSourceMake}
                    onChange={(e) => setHeatSourceMake(e.target.value)}
                    className="w-full px-4 py-3 border-2 border-gray-300 rounded-lg focus:border-indigo-500 focus:outline-none" />
                </div>
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-2">Model</label>
                  <input type="text" placeholder="e.g. Greenstar 30i" value={heatSourceModel}
                    onChange={(e) => setHeatSourceModel(e.target.value)}
                    className="w-full px-4 py-3 border-2 border-gray-300 rounded-lg focus:border-indigo-500 focus:outline-none" />
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
                <div>
                  ...
                  <select value={heatSourceCapacity} onChange={(e) => setHeatSourceCapacity(e.target.value)}
                    className="w-full px-4 py-3 border-2 border-gray-300 rounded-lg focus:border-indigo-500 focus:outline-none">
                    <option value="">Select capacity</option>
                    {BOILER_SIZES.map(s => <option key={s} value={s}>{s} kW</option>)}
                  </select>
                </div>
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-2">Condition</label>
                  <select value={heatSourceCondition} onChange={(e) => setHeatSourceCondition(e.target.value)}
                    className="w-full px-4 py-3 border-2 border-gray-300 rounded-lg focus:border-indigo-500 focus:outline-none">
                    {Object.keys(HEAT_SOURCE_CONDITIONS).map(c => <option key={c} value={c}>{c}</option>)}
                  </select>
                </div>
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-2">Age (years)</label>
                  <input type="number" placeholder="e.g. 5" value={heatSourceAge}
                    onChange={(e) => setHeatSourceAge(e.target.value)}
                    className="w-full px-4 py-3 border-2 border-gray-300 rounded-lg focus:border-indigo-500 focus:outline-none" />
                </div>
              </div>

            {/* System Adequacy Assessment - shows automatically when rooms have data */}
              {systemAdequacy.totalHeatRequired > 0 && (
                <div className={`p-4 rounded-lg border-2 ${
                  systemAdequacy.statusColor === 'green' ? 'bg-green-50 border-green-300' :
                  systemAdequacy.statusColor === 'yellow' ? 'bg-yellow-50 border-yellow-300' :
                  systemAdequacy.statusColor === 'orange' ? 'bg-orange-50 border-orange-300' :
                  systemAdequacy.statusColor === 'red' ? 'bg-red-50 border-red-300' :
                  'bg-gray-50 border-gray-300'
              }`}>
                  <h4 className="font-semibold text-gray-800 mb-2">System Adequacy Assessment</h4>
      
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-3">
                    <div>
                      <p className="text-xs text-gray-600">Total Heat Required</p>
                      <p className="text-lg font-bold">{(systemAdequacy.totalHeatRequired / 1000).toFixed(2)} kW</p>
                    </div>
                    <div>
                      <p className="text-xs text-gray-600">Hot Water Allowance</p>
                      <p className="text-lg font-bold">{(systemAdequacy.hotWaterAllowance / 1000).toFixed(1)} kW</p>
                    </div>
                    <div>
                      <p className="text-xs text-gray-600">Total System Demand</p>
                      <p className="text-lg font-bold text-blue-600">{(systemAdequacy.totalSystemDemand / 1000).toFixed(2)} kW</p>
                    </div>
                    <div>
                      <p className="text-xs text-gray-600">Effective Capacity</p>
                      <p className="text-lg font-bold">{(systemAdequacy.effectiveCapacity / 1000).toFixed(1)} kW</p>
                    </div>
                  </div>
      
                  <div className="flex items-center gap-3 mb-2">
                    <div className={`px-3 py-1 rounded-full text-sm font-bold ${
                      systemAdequacy.statusColor === 'green' ? 'bg-green-500 text-white' :
                      systemAdequacy.statusColor === 'yellow' ? 'bg-yellow-500 text-white' :
                      systemAdequacy.statusColor === 'orange' ? 'bg-orange-500 text-white' :
                      systemAdequacy.statusColor === 'red' ? 'bg-red-500 text-white' :
                      'bg-gray-500 text-white'
                  }`}>
                      {systemAdequacy.status}
                    </div>
                    {systemAdequacy.capacityUsed > 0 && (
                      <span className="text-sm text-gray-600">
                        {systemAdequacy.capacityUsed.toFixed(0)}% of effective capacity used
                        </span>
                    )}
                  </div>
      
                  <p className="text-sm text-gray-700">{systemAdequacy.recommendation}</p>
                  {systemAdequacy.conditionNote && (
                    <p className="text-xs text-gray-500 mt-1">Condition note: {systemAdequacy.conditionNote}</p>
                  )}
      
                  {heatSourceType.includes('Heat Pump') && (
                    <div className="mt-3 p-3 bg-blue-100 rounded-lg">
                      <p className="text-sm text-blue-800">
                        <strong>Heat Pump Note:</strong> Maximum recommended flow temperature is 55°C. 
            Ensure radiators are sized for lower ΔT. Current system ΔT: {rooms[0]?.deltaT || 50}
                      </p>
                    </div>
                  )}
                </div>
      ...
      {/* Footer */}
          <div className="text-center text-sm text-gray-500 py-4 mb-20">
            <p> 
              This app was coded by Claude and Maria de la Serna. It is available on
              <a href="https://github.com/MariaBego56/heat-loss-calculator" target="_blank" rel="noopener noreferrer" className="text-indigo-600 hover:underline"> GitHub </a> and hosted on <a href=" https://heatlosscalculator.netlify.app/" target="blank" rel="noopener noreferrer">Netlify</a>, January 2026
            </p>
          </div>
        ))}
...
// If needed this list can be upated or expanded
...
const HEAT_SOURCE_TYPES = {
  'Combi Boiler': { hotWaterAllowanceKW: 3.0 },
  'System Boiler (Cylinder)': { hotWaterAllowanceKW: 2.0 },
  'Regular/Conventional Boiler (Tank + Cylinder)': { hotWaterAllowanceKW: 2.0 },
  'Air Source Heat Pump': { hotWaterAllowanceKW: 2.5 },
  'Ground Source Heat Pump': { hotWaterAllowanceKW: 2.5 },
  'Electric Boiler': { hotWaterAllowanceKW: 2.0 },
  'Other': { hotWaterAllowanceKW: 2.0 }
};

const HEAT_SOURCE_CONDITIONS = {
  'Excellent': { factor: 1.0, note: '' },
  'Good': { factor: 0.95, note: '' },
  'Fair': { factor: 0.85, note: 'Capacity de-rated for condition.' },
  'Poor': { factor: 0.75, note: 'Capacity significantly de-rated for condition.' }
};
// keep simple: a reasonable UK-like list. This can be adapted to other requirements
const BOILER_SIZES = [12, 15, 18, 21, 24, 28, 30, 32, 35, 38, 40, 45, 50];
...
// Floor types with U-values (W/m²K) and temperature differentials
const FLOOR_TYPES = {
...
// Ceiling/Roof types with U-values (W/m²K) and temperature differentials
const CEILING_TYPES = {
...
// Function to lookup radiator output (nearest size + ratio scaling)
...
function App() {
  // --- helper placed BEFORE useState initialiser (keeps everything happy) ---
...
// Mean water temperature
  ...
  const [heatSourceType, setHeatSourceType] = useState('Combi Boiler');
  const [heatSourceMake, setHeatSourceMake] = useState('');
  const [heatSourceModel, setHeatSourceModel] = useState('');
  const [heatSourceCapacity, setHeatSourceCapacity] = useState('');
  const [heatSourceCondition, setHeatSourceCondition] = useState('Good');
  const [heatSourceAge, setHeatSourceAge] = useState('');

  
  const [rooms, setRooms] = useState(() => [createEmptyRoom()]);

  const systemAdequacy = useMemo(() => {
    const totalHeatRequired = rooms.reduce((sum, r) => sum + (parseFloat(r.requiredHeat) || 0), 0);

    const capKW = parseFloat(heatSourceCapacity) || 0;
    const conditionData = HEAT_SOURCE_CONDITIONS[heatSourceCondition] || { factor: 0.9, note: '' };
    const effectiveCapacity = capKW * 1000 * conditionData.factor; // W

    const hotWaterAllowanceKW = HEAT_SOURCE_TYPES[heatSourceType]?.hotWaterAllowanceKW ?? 2.0;
    const hotWaterAllowance = hotWaterAllowanceKW * 1000; // W

    const totalSystemDemand = totalHeatRequired + hotWaterAllowance; // W
  // status + colour (kept simple)
    let status = 'Not assessed';
    let statusColor = 'gray';
    let recommendation = 'Enter heat source capacity to assess adequacy.';

    if (totalHeatRequired > 0 && effectiveCapacity > 0) {
      const used = (totalSystemDemand / effectiveCapacity) * 100;

      if (used <= 85) {
        status = 'Adequate';
        statusColor = 'green';
        recommendation = 'Heat source capacity appears adequate with a sensible margin.';
      } else if (used <= 100) {
        status = 'Borderline';
        statusColor = 'yellow';
        recommendation = 'Capacity is borderline. Consider reduced flow temperatures, upgrades, or improved insulation.';
      } else if (used <= 120) {
        status = 'Likely undersized';
        statusColor = 'orange';
        recommendation = 'Capacity likely undersized for peak conditions. Review emitter sizing and heat source specification.';
      } else {
        status = 'Undersized';
        statusColor = 'red';
        recommendation = 'Capacity is undersized. Heat source upgrade or major fabric improvements recommended.';
      }
    }

    const capacityUsed =
      totalHeatRequired > 0 && effectiveCapacity > 0 ? (totalSystemDemand / effectiveCapacity) * 100 : 0;
    ...
    return {
      ...
      statusColor,
      recommendation,
      conditionNote: conditionData.note,
      capacityUsed
    };
  }, [rooms, heatSourceCapacity, heatSourceCondition, heatSourceType]);
 ...
 // Apply window condition factor to U-value
          ...
          const doorLoss = door1A * door1U * tempDiff + door2A * door2U * tempDiff;
          ...
          if (floorData) {
            if (floorData.tempDiff === 0) {
              ...
              const groundTempDiff = (parseFloat(u.requiredTemp) || 21) - 10;
          ...
          const fabricNoBridge = wallLoss + winLoss + doorLoss + floorLoss + ceilingLoss;
          u.fabricHeatLoss = fabricNoBridge.toFixed(0);
          ...
          // Thermal bridging + intermittent uplift 
          const thermalBridgingFactor = BUILDING_AGE[buildingAge] || 0.1;
          const thermalBridgingLoss = fabricNoBridge * thermalBridgingFactor;

          const subtotal =
            fabricNoBridge + thermalBridgingLoss + (parseFloat(u.ventilationHeatLoss) || 0);
  ...
  const generateExcel = () => {
    ...
    data.push([
      ...
      ...rooms.map((r) =>
        r.requiredTemp && r.designTemp ? (parseFloat(r.requiredTemp) - parseFloat(r.designTemp)).toFixed(0) : ''
      )
    ]);
    data.push([]);
    ...
    data.push([
      'Condition Factor',
      ...rooms.map((r) => (r.windowCondition && WINDOW_CONDITIONS[r.windowCondition] ? WINDOW_CONDITIONS[r.windowCondition].factor + 'x' : ''))
    ]);
    ...
    // Notes sheet includes boiler/heat source + adequacy in Notes/Recommendation section
    const notes = [
      ['HEAT LOSS CALCULATION NOTES AND GUIDANCE'],
      [],
      ['HEAT SOURCE & SYSTEM ADEQUACY (NOTES / RECOMMENDATION)'],
      ['Heat source type:', heatSourceType || '-'],
      ['Make:', heatSourceMake || '-'],
      ['Model:', heatSourceModel || '-'],
      ['Capacity (kW):', heatSourceCapacity || '-'],
      ['Condition:', heatSourceCondition || '-'],
      ['Age (years):', heatSourceAge || '-'],
      [],
      ['Total heat required (kW):', systemAdequacy.totalHeatRequired ? (systemAdequacy.totalHeatRequired / 1000).toFixed(2) : '-'],
      ['Hot water allowance (kW):', systemAdequacy.hotWaterAllowance ? (systemAdequacy.hotWaterAllowance / 1000).toFixed(2) : '-'],
      ['Total system demand (kW):', systemAdequacy.totalSystemDemand ? (systemAdequacy.totalSystemDemand / 1000).toFixed(2) : '-'],
      ['Effective capacity (kW):', systemAdequacy.effectiveCapacity ? (systemAdequacy.effectiveCapacity / 1000).toFixed(2) : '-'],
      ['Capacity used (%):', systemAdequacy.capacityUsed ? systemAdequacy.capacityUsed.toFixed(0) + '%' : '-'],
      ['Status:', systemAdequacy.status || '-'],
      ['Recommendation:', systemAdequacy.recommendation || '-'],
      ...(systemAdequacy.conditionNote ? [['Condition note:', systemAdequacy.conditionNote]] : []),
      [],
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="bg-white rounded-lg shadow-lg p-6 mb-6">
          ...
          <div className="space-y-4">
            ...
            {/* ✅ Fixed nesting: Building Age + Heating Pattern in same grid (your earlier error #3 fix) */}
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-4">
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-2">Building Age</label>
                <select
                  ...
                  className="w-full px-4 py-3 border-2 border-gray-300 rounded-lg focus:border-indigo-500 focus:outline-none"
                >
                  {Object.keys(BUILDING_AGE).map((age) => (
              ...
              <div>
                ...
                <select
                  ...
                  onChange={(e) => setHeatingPattern(e.target.value)}
                  className="w-full px-4 py-3 border-2 border-gray-300 rounded-lg focus:border-indigo-500 focus:outline-none"
                >
                  {Object.keys(HEATING_PATTERN).map((p) => (
            ...
            <div className="border-t-2 pt-4 mt-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-2">Heat Source Type</label>
                  <select
                    value={heatSourceType}
                    onChange={(e) => setHeatSourceType(e.target.value)}
                    className="w-full px-4 py-3 border-2 border-gray-300 rounded-lg focus:border-indigo-500 focus:outline-none"
                  >
                    {Object.keys(HEAT_SOURCE_TYPES).map((t) => (
                      <option key={t} value={t}>
                        {t}
                      </option>
                    ))}
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-2">Capacity (kW)</label>
                  <select
                    value={heatSourceCapacity}
                    onChange={(e) => setHeatSourceCapacity(e.target.value)}
                    className="w-full px-4 py-3 border-2 border-gray-300 rounded-lg focus:border-indigo-500 focus:outline-none"
                  >
                    <option value="">Select capacity</option>
                    {BOILER_SIZES.map((s) => (
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-2">Condition</label>
                  <select
                    value={heatSourceCondition}
                    onChange={(e) => setHeatSourceCondition(e.target.value)}
                    className="w-full px-4 py-3 border-2 border-gray-300 rounded-lg focus:border-indigo-500 focus:outline-none"
                  >
                    {Object.keys(HEAT_SOURCE_CONDITIONS).map((c) => (
              ...
              {systemAdequacy.totalHeatRequired > 0 && (
                <div
                  className={`p-4 rounded-lg border-2 ${
                    systemAdequacy.statusColor === 'green'
                      ? 'bg-green-50 border-green-300'
                      : systemAdequacy.statusColor === 'yellow'
                      ? 'bg-yellow-50 border-yellow-300'
                      : systemAdequacy.statusColor === 'orange'
                      ? 'bg-orange-50 border-orange-300'
                      : systemAdequacy.statusColor === 'red'
                ...
                >
                  ...
                  <div className="flex items-center gap-3 mb-2">
                    ...
                    {systemAdequacy.capacityUsed > 0 && (
                      <span className="text-sm text-gray-600">{systemAdequacy.capacityUsed.toFixed(0)}% of effective capacity used</span>
                    )}
                  </div>

                  <p className="text-sm text-gray-700">{systemAdequacy.recommendation}</p>
                  {systemAdequacy.conditionNote && <p className="text-xs text-gray-500 mt-1">Condition note: {systemAdequacy.conditionNote}</p>}

                  {heatSourceType.includes('Heat Pump') && (
                    <div className="mt-3 p-3 bg-blue-100 rounded-lg">
                      <p className="text-sm text-blue-800">
                        <strong>Heat Pump Note:</strong> Maximum recommended flow temperature is 55°C. Ensure radiators are sized for lower ΔT.
                        Current system ΔT: {rooms[0]?.deltaT || 50}
                      </p>
                    </div>
                  )}
                </div>
            ...
            <div className="space-y-4">
                ...
                <div className="grid grid-cols-2 gap-3">
                  ...
                  <div>
                    ...
                    <select
                      ...
                      onChange={(e) => updateRoom(room.id, 'designTemp', e.target.value)}
                      className="w-full px-4 py-3 border-2 border-gray-300 rounded-lg focus:border-indigo-500 focus:outline-none"
                    >
                      <option value="-3">-3°C (UK Standard)</option>
                      <option value="-10">-10°C (Cold Regions)</option>
              ...
              <div className="border-t-2 pt-4">
                ...
                <div className="mb-4">
                  ...
                  <select
                    value={room.wallType}
                    onChange={(e) => updateRoom(room.id, 'wallType', e.target.value)}
                    className="w-full px-4 py-3 border-2 border-gray-300 rounded-lg focus:border-indigo-500 focus:outline-none"
                  >
                    <option value="">Select wall type</option>
                    {Object.keys(WALL_TYPES).map((t) => (
                      <option key={t} value={t}>
                        {t} {WALL_TYPES[t] > 0 ? `(U=${WALL_TYPES[t]} W/m²K)` : ''}
                      </option>
                    ))}
                  </select>
                </div>
                ...
                {room.wallType === 'No External Wall' ? (
                  <div className="bg-gray-50 p-4 rounded-lg border-2 border-gray-200">
                    <p className="text-sm text-gray-600">
                      ℹ️ This room has no external walls - wall heat loss will be zero. Heat loss will only be calculated from windows, doors, floor and ceiling.
                    </p>
                  </div>
                ) : (
                  <>
              ...
              <div className="border-t-2 pt-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                  <div>
                    ...
                    >
                      <option value="">Select window type</option>
                      {Object.keys(WINDOW_TYPES).map((t) => (
                        <option key={t} value={t}>
                          {t} (U={WINDOW_TYPES[t]} W/m²K)
                        </option>
                      ))}
                    </select>
                  </div>
                  ...
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-2">Window Condition</label>
                    <select
                      value={room.windowCondition}
                      onChange={(e) => updateRoom(room.id, 'windowCondition', e.target.value)}
                      className="w-full px-4 py-3 border-2 border-gray-300 rounded-lg focus:border-indigo-500 focus:outline-none"
                    >
                      <option value="">Select condition</option>
                      {Object.keys(WINDOW_CONDITIONS).map((t) => (
              ...
              <div className="border-t-2 pt-4">
                ...
                <div className="mb-4">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-4 gap-3">
                    <div>
                      <select
                        value={room.door1Type}
                        onChange={(e) => updateRoom(room.id, 'door1Type', e.target.value)}
                        className="w-full px-3 py-2 border-2 border-gray-300 rounded-lg focus:border-indigo-500 focus:outline-none text-sm"
                      >
                        <option value="">Select door type</option>
                        {Object.keys(DOOR_TYPES).map((t) => (
                          <option key={t} value={t}>
                            {t} (U={DOOR_TYPES[t]})
                          </option>
                        ))}
                ...
                <div className="mb-4">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-4 gap-3">
                    <div>
                      <select
                        value={room.door2Type}
                        onChange={(e) => updateRoom(room.id, 'door2Type', e.target.value)}
                        className="w-full px-3 py-2 border-2 border-gray-300 rounded-lg focus:border-indigo-500 focus:outline-none text-sm"
                      >
                        <option value="">Select door type</option>
                        {Object.keys(DOOR_TYPES).map((t) => (
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    ...
                    <select
                      ...
                      onChange={(e) => updateRoom(room.id, 'floorType', e.target.value)}
                      className="w-full px-4 py-3 border-2 border-gray-300 rounded-lg focus:border-indigo-500 focus:outline-none"
                    >
                      <option value="">Select floor type</option>
                      {Object.keys(FLOOR_TYPES).map((t) => (
                    ...
                    {room.floorType && FLOOR_TYPES[room.floorType]?.uValue >= 0.7 && <p className="text-xs text-orange-600 mt-1">⚠ High heat loss floor - consider insulation</p>}
                  </div>
                  ...
                  <div>
                    ...
                    <select
                      ...
                      onChange={(e) => updateRoom(room.id, 'ceilingType', e.target.value)}
                      className="w-full px-4 py-3 border-2 border-gray-300 rounded-lg focus:border-indigo-500 focus:outline-none"
                    >
                      <option value="">Select ceiling type</option>
                      {Object.keys(CEILING_TYPES).map((t) => (
                    ...
                    {room.ceilingType && CEILING_TYPES[room.ceilingType]?.uValue >= 1.0 && <p className="text-xs text-orange-600 mt-1">⚠ High heat loss ceiling - consider insulation</p>}
                  </div>
                </div>
              </div>
              ...
              <div className="border-t-2 pt-4">
                <div className="bg-blue-50 p-4 rounded-lg">
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                    ...
                    <div>
                      <label className="block text-xs font-medium text-gray-700 mb-1">Mean Water Temp</label>
                      <input type="text" value={room.mwt + '°C'} readOnly className="w-full px-3 py-2 border-2 border-gray-200 rounded-lg bg-gray-100 font-medium" />
                    </div>
                    <div>
                      <label className="block text-xs font-medium text-gray-700 mb-1">Delta T</label>
                      <input type="text" value={'ΔT' + room.deltaT} readOnly className="w-full px-3 py-2 border-2 border-gray-200 rounded-lg bg-yellow-100 font-bold text-center" />
                    </div>
                  </div>
                  <p className="text-xs text-gray-600 mt-2">Standard: Flow 75°C, Return 65°C = MWT 70°C, ΔT50. Adjust if using heat pump or different system.</p>
                </div>
              </div>
              ...
              <div className="border-t-2 pt-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-3">
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-2">Radiator Type</label>
                    <select
                      value={room.existRadType}
                      onChange={(e) => updateRoom(room.id, 'existRadType', e.target.value)}
                      className="w-full px-4 py-3 border-2 border-gray-300 rounded-lg focus:border-indigo-500 focus:outline-none"
                    >
                      <option value="">Select type</option>
                  ...
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-2">Height (mm)</label>
                    <select
                      value={room.existRadH}
                      onChange={(e) => updateRoom(room.id, 'existRadH', e.target.value)}
                      className="w-full px-4 py-3 border-2 border-gray-300 rounded-lg focus:border-indigo-500 focus:outline-none"
                    >
                      <option value="">Select height</option>
                      <option value="300">300mm</option>
                  ...
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-2">Length (mm)</label>
                    <select
                      value={room.existRadL}
                      onChange={(e) => updateRoom(room.id, 'existRadL', e.target.value)}
                      className="w-full px-4 py-3 border-2 border-gray-300 rounded-lg focus:border-indigo-500 focus:outline-none"
                    >
                      <option value="">Select length</option>
                      {[400, 500, 600, 700, 800, 900, 1000, 1100, 1200, 1400, 1600, 1800, 2000].map((n) => (
                ...
                {room.existRadActual && room.requiredHeat && (
                  <div
                    className={`mt-4 p-4 rounded-lg border-2 ${
                      parseFloat(room.existRadActual) >= parseFloat(room.requiredHeat) ? 'bg-green-100 border-green-300' : 'bg-red-100 border-red-300'
                    }`}
                  >
                    <div className="flex items-center gap-2 mb-2">
                      {parseFloat(room.existRadActual) >= parseFloat(room.requiredHeat) ? (
                    ...
                    {parseFloat(room.existRadActual) < parseFloat(room.requiredHeat) && (
                      <div className="mt-2 pt-2 border-t border-red-200">
                        <p className="text-gray-600 text-sm">Shortfall:</p>
                        <p className="font-bold text-red-700">{(parseFloat(room.requiredHeat) - parseFloat(room.existRadActual)).toFixed(0)} W deficit</p>
                      </div>
                    )}
                  </div>
                )}
              ...
              <div className="border-t-2 pt-4">
                ...
                <div className="mb-4">
                  <label className="block text-sm font-medium text-gray-700 mb-2">Action Required</label>
                  <select
                    value={room.recommendation}
                    onChange={(e) => updateRoom(room.id, 'recommendation', e.target.value)}
                    className="w-full px-4 py-3 border-2 border-gray-300 rounded-lg focus:border-indigo-500 focus:outline-none"
                  >
                    <option value="Keep as is">Keep as is - No action required</option>
                    <option value="Replace radiator">Replace existing radiator</option>
                  ...
                  <div className="bg-yellow-50 p-4 rounded-lg border-2 border-yellow-200">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-3">
                      <div>
                        <label className="block text-sm font-medium text-gray-700 mb-2">Radiator Type</label>
                        <select
                          value={room.recRadType}
                          onChange={(e) => updateRoom(room.id, 'recRadType', e.target.value)}
                          className="w-full px-4 py-3 border-2 border-gray-300 rounded-lg focus:border-indigo-500 focus:outline-none"
                        >
                          <option value="">Select type</option>
                          {RADIATOR_TYPES.map((t) => (
                      ...
                      <div>
                        ...
                        <select
                          value={room.recRadH}
                          onChange={(e) => updateRoom(room.id, 'recRadH', e.target.value)}
                          className="w-full px-4 py-3 border-2 border-gray-300 rounded-lg focus:border-indigo-500 focus:outline-none"
                        >
                          <option value="">Select height</option>
                          <option value="300">300mm</option>
                      ...
                      <div>
                        ...
                        <select
                          value={room.recRadL}
                          onChange={(e) => updateRoom(room.id, 'recRadL', e.target.value)}
                          className="w-full px-4 py-3 border-2 border-gray-300 rounded-lg focus:border-indigo-500 focus:outline-none"
                        >
                          <option value="">Select length</option>
                          {[400, 500, 600, 700, 800, 900, 1000, 1100, 1200, 1400, 1600, 1800, 2000].map((n) => (
        ...
        {/* Footer (outside rooms map) */}
        <div className="text-center text-sm text-gray-500 py-4 mb-20">
          <p>
            This app was coded by Claude and Maria de la Serna. It is available on{' '}
            <a
              href="https://github.com/MariaBego56/heat-loss-calculator"
              target="_blank"
            ...
            and hosted on{' '}
            <a href="https://heatlosscalculator.netlify.app/" target="_blank" rel="noopener noreferrer" className="text-indigo-600 hover:underline">
              Netlify
            </a>
            , January 2026
          </p>
